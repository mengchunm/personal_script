...
'@
    ...
    # 保存Python脚本 (使用UTF-8无BOM编码，避免Python报错)
    $scriptPath = "$script:scriptDir\login_script.py"
    $utf8NoBom = New-Object System.Text.UTF8Encoding $false
    [System.IO.File]::WriteAllText($scriptPath, $pythonScript, $utf8NoBom)
    ...
    # 创建批处理文件（添加错误处理、自动安装依赖和日志）
    $batContent = @"
@echo off
cd /d "$script:scriptDir"
set LOGFILE=login_script_python.log
set PYTHON_EXE=
set PIP_CMD=
...
if exist "python\python.exe" (
    ...
    echo [INFO] Using local Python >> "%LOGFILE%"
) else (
    where python.exe >nul 2>&1
    if not errorlevel 1 (
        set "PYTHON_EXE=python.exe"
        set "PIP_CMD=pip"
        echo [INFO] Using system Python >> "%LOGFILE%"
    ) else (
        echo [ERROR] No Python found! >> "%LOGFILE%"
        exit /b 1
    )
)
...
if errorlevel 1 (
    echo [INFO] Installing requests... >> "%LOGFILE%"
    %PIP_CMD% install requests >> "%LOGFILE%" 2>&1
)
%PYTHON_EXE% -c "import bs4" >nul 2>&1
if errorlevel 1 (
    echo [INFO] Installing beautifulsoup4... >> "%LOGFILE%"
    %PIP_CMD% install beautifulsoup4 >> "%LOGFILE%" 2>&1
)

REM 运行登录脚本
echo [INFO] Starting login script... >> "%LOGFILE%"
if exist "python\pythonw.exe" (
    python\pythonw.exe login_script.py >> "%LOGFILE%" 2>&1
) else (
    %PYTHON_EXE% login_script.py >> "%LOGFILE%" 2>&1
)
"@
...
function Install-PythonEnvironment {
    ...
    if (Test-Path "$pythonDir\python.exe") {
        ...
        if (Test-Path $pipPath) {
            Write-LogMessage "正在检查已安装的包..."
            
            # 检查是否已安装必需的包
            $installedPackages = & $pipPath list 2>&1
            $hasRequests = $installedPackages -match "requests"
            $hasBeautifulSoup = $installedPackages -match "beautifulsoup4"
            
            if ($hasRequests -and $hasBeautifulSoup) {
                Write-LogMessage "所有依赖包已安装" -Level "SUCCESS"
                return $true
            }
            
            # 安装缺失的包
            Write-LogMessage "安装/更新依赖包 (requests, beautifulsoup4)..."
            $installOutput = & $pipPath install --upgrade requests beautifulsoup4 2>&1
            
            if ($LASTEXITCODE -eq 0) {
                Write-LogMessage "依赖包安装成功" -Level "SUCCESS"
                ...
                if ($hasRequests -and $hasBeautifulSoup) {
                    Write-LogMessage "验证通过: 所有依赖包已正确安装" -Level "SUCCESS"
                    return $true
                }
                else {
                    Write-LogMessage "警告: 依赖包安装可能未成功" -Level "WARNING"
                    if (-not $hasRequests) { Write-LogMessage "  - requests 未找到" -Level "WARNING" }
                    if (-not $hasBeautifulSoup) { Write-LogMessage "  - beautifulsoup4 未找到" -Level "WARNING" }
                }
            }
            else {
                Write-LogMessage "依赖包安装失败，详细信息:" -Level "ERROR"
                Write-LogMessage $installOutput -Level "ERROR"
            }
        }
    }
    ...
    try {
        ...
        # 修复python._pth文件 - 这是关键步骤！
        Write-LogMessage "配置Python环境以支持pip..."
        $pthFile = "$pythonDir\python311._pth"
        if (Test-Path $pthFile) {
            $pthContent = Get-Content $pthFile
            # 取消注释 import site 以启用 site-packages
            $pthContent = $pthContent -replace '^#import site', 'import site'
            $pthContent | Set-Content $pthFile
            Write-LogMessage "已启用site-packages支持" -Level "SUCCESS"
        }
        else {
            Write-LogMessage "警告: 未找到 python311._pth 文件" -Level "WARNING"
        }
        ...
        $pipInstallOutput = & "$pythonDir\python.exe" "$getpipPath" 2>&1
        if ($LASTEXITCODE -ne 0) {
            Write-LogMessage "pip安装失败:" -Level "ERROR"
            Write-LogMessage $pipInstallOutput -Level "ERROR"
            throw "pip安装失败"
        }
        Write-LogMessage "pip安装成功" -Level "SUCCESS"

        # 安装依赖包
        Write-LogMessage "安装依赖包 (requests, beautifulsoup4)..."
        $pipPath = "$pythonDir\Scripts\pip.exe"
        
        if (Test-Path $pipPath) {
            Write-LogMessage "使用 pip.exe 安装..."
            $depsInstallOutput = & $pipPath install requests beautifulsoup4 2>&1
        }
        else {
            Write-LogMessage "使用 python -m pip 安装..."
            $depsInstallOutput = & "$pythonDir\python.exe" -m pip install requests beautifulsoup4 2>&1
        }
        
        if ($LASTEXITCODE -ne 0) {
            Write-LogMessage "依赖包安装失败:" -Level "ERROR"
            Write-LogMessage $depsInstallOutput -Level "ERROR"
        }
        else {
            Write-LogMessage "依赖包安装命令执行成功" -Level "SUCCESS"
        }

        # 验证安装结果
        Write-LogMessage "验证依赖包安装..."
        if (Test-Path $pipPath) {
            $installedPackages = & $pipPath list 2>&1
        }
        else {
            $installedPackages = & "$pythonDir\python.exe" -m pip list 2>&1
        }
        ...
        if ($hasRequests -and $hasBeautifulSoup) {
            Write-LogMessage "✓ requests 已安装" -Level "SUCCESS"
            Write-LogMessage "✓ beautifulsoup4 已安装" -Level "SUCCESS"
        }
        else {
            if (-not $hasRequests) { 
                Write-LogMessage "✗ requests 未安装" -Level "ERROR" 
            }
            else {
                Write-LogMessage "✓ requests 已安装" -Level "SUCCESS"
            }
            if (-not $hasBeautifulSoup) { 
                Write-LogMessage "✗ beautifulsoup4 未安装" -Level "ERROR" 
            }
            else {
                Write-LogMessage "✓ beautifulsoup4 已安装" -Level "SUCCESS"
            }
            
            Write-LogMessage "警告: 部分依赖包未成功安装，自动登录可能无法正常工作" -Level "WARNING"
        }
        ...
        # 尝试使用系统Python
        Write-LogMessage "检查系统Python..." -Level "WARNING"
        $pythonCmd = Get-Command python -ErrorAction SilentlyContinue
        $pythonwCmd = Get-Command pythonw -ErrorAction SilentlyContinue
        
        if ($pythonCmd -or $pythonwCmd) {
            Write-LogMessage "找到系统Python，正在验证依赖..." -Level "WARNING"
            
            # 验证系统Python是否有所需依赖
            $pythonExe = if ($pythonCmd) { $pythonCmd.Path } else { $pythonwCmd.Path }
            $checkDeps = & $pythonExe -c "import requests; import bs4; print('OK')" 2>&1
            
            if ($checkDeps -match "OK") {
                Write-LogMessage "系统Python依赖验证通过" -Level "SUCCESS"
                
                # 更新批处理文件使用系统Python
                $systemPythonw = if ($pythonwCmd) { $pythonwCmd.Path } else { $pythonCmd.Path }
                $batContent = @"
@echo off
cd /d "$script:scriptDir"
set LOGFILE=login_script_python.log

"$systemPythonw" login_script.py >> "%LOGFILE%" 2>&1
if errorlevel 1 (
    echo [ERROR] Script execution failed! >> "%LOGFILE%"
    exit /b 1
)
"@
                $batPath = "$script:scriptDir\run_login.bat"
                $batContent | Out-File -FilePath $batPath -Encoding ASCII -Force
                Write-LogMessage "批处理文件已更新为使用系统Python: $systemPythonw" -Level "SUCCESS"
                
                return $true
            }
            else {
                Write-LogMessage "系统Python缺少依赖包，请手动安装: pip install requests beautifulsoup4" -Level "ERROR"
                return $false
            }
        }

        Write-LogMessage "未找到可用的Python环境" -Level "ERROR"
        return $false
    }
}
...
function Start-MainProgram {
    ...
    # 如果是通过参数直接进入自动登录安装
    if ($DirectToAutoLogin) {
        Write-Host "正在进入自动登录安装...`n" -ForegroundColor Cyan
        Install-AutoLogin
        # 安装完成后，如果是提升权限的会话，继续显示菜单
        if ($ElevatedSession) {
            Write-Host "`n您现在可以继续使用其他功能，或选择退出" -ForegroundColor Cyan
        }
    }
